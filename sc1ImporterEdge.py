    ...
    with open(filepath, "rb") as file:
        ...
        for bone_index, submesh in enumerate(submeshes):
            ...
            for vertex_idx in range(submesh["vertex_count"]+submesh["corrective_count"]):
                ...
                if vertex_index < 32768:
                    ...
                    base_vertices[target_index] = transformed_vertex.xyz
        ...
        while(1):
            ...
            for i in range(len(triangles) - 2):
                try:
                    v1 = bm.verts.new(base_vertices[triangles[i]])
                    v2 = bm.verts.new(base_vertices[triangles[i + 1]])
                    v3 = bm.verts.new(base_vertices[triangles[i + 2]])
                    if not vertexFix.get(triangles[i]):
                        vertexFix[triangles[i]] = []
                    vertexFix[triangles[i]].append(v1.index)
                    if not vertexFix.get(triangles[i+1]):
                        vertexFix[triangles[i+1]] = []
                    vertexFix[triangles[i + 1]].append(v2.index)
                    if not vertexFix.get(triangles[i+2]):
                        vertexFix[triangles[i+2]] = []
                    vertexFix[triangles[i + 2]].append(v3.index)
        ...
        for bone_index, vertex_indices in bone_vertex_indices.items():
            ...
            if vertex_indices:  # Only add if there are vertices
                for indxzz in vertex_indices:
                    indexesz.extend(vertexFix[indxzz])
                vertex_group = obj.vertex_groups[f"Bone_{bone_index}"]
                vertex_group.add(indexesz, 1.0, 'REPLACE')
