...
def import_soul_calibur_model(filepath):
    ...
    with open(filepath, "rb") as file:
        ...
        for x in range(submesh_count):
            vertex_count_lone = read_little_endian_uint16(file) 
            vertex_count_shared = read_little_endian_uint16(file)
            vertex_list_pointer = read_little_endian_uint32(file)
            unknown = read_little_endian_uint32(file)#Extra do dads
            ...
            submeshes.append({
                "vertex_count": vertex_count_lone,
                "corrective_count": vertex_count_shared,
        ...
        for bone_index, submesh in enumerate(submeshes):
            ...
            for vertex_idx in range(submesh["vertex_count"]+submesh["corrective_count"]):
                ...
                vertex_index = read_little_endian_int16(file)

                w_component = read_little_endian_int16(file)
                w_component <<=16
                # Extract vertex index from w component's upper bits
                w_bytes = struct.pack('f', w_component)[0]
                
                # Get target vertex index (12 bits)/Lets go beon
                vertexDirect = vertex_index & 0x7FFF
                vertexShared = vertex_index >> 15

                
                vertex = Vector((vertex_x, vertex_y, vertex_z, w_bytes))
                transformed_vertex = bone_matrix @ vertex


                if vertexShared == 0:
                    print(f"Vertex: {vertex_index}")
                    base_vertices[vertexDirect].co = transformed_vertex.xyz
                else:
                    print(f"Corrective: {vertex_index}")
                    base_vertices[vertexDirect].co += transformed_vertex.xyz
                weghts[vertexDirect] = w_bytes
                print(f"Vertex: {vertex_index}")
                weghts[vertexDirect] = w_bytes
            bone_vertex_indices[bone_index]= weghts
        ...
        while(1):
            ...
            for i in range(len(triangles) - 2):
                try:
                    v1 = base_vertices[triangles[i]]
                    v2 = base_vertices[triangles[i + 1]]
                    v3 = base_vertices[triangles[i + 2]]
                    u1 = uvtri[i]
                    u2 = uvtri[i+1]
                    u3 = uvtri[i+2]
                    ...
                    cur = bm.faces.new(face_verts)
                    cur.loops[0][uv_layer].uv = uv_verts[0]
                    cur.loops[1][uv_layer].uv = uv_verts[1]
                    cur.loops[2][uv_layer].uv = uv_verts[2]

                    cur.material_index = currMats.index(mat_idx)
        ...
        for bone_index, vertex_indices in bone_vertex_indices.items():
            for vertex_index, weight in vertex_indices.items():
